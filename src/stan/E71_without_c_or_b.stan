...
functions{
  vector logistic(real a, int N_len, vector concentration){
    vector[N_len] log_denom = log(1 + exp(-(a + concentration)));
...
model {
  ...
  survival ~ binomial(nreplicates, logistic(a, N, concentration));
...
generated quantities{
  vector[N_sim] prob;
  {
    ...
    prob = logistic(a, N_sim, concentration);
  }
}
