...
model = stan_model("src/stan/ev_data/E71_model_fitAll_ppc_24Feb23.stan")
opt <- rstan::optimizing(model, data = stan_data, init = 'random', as_vector = FALSE)
initf <- function() { 
  list( a = opt$par$a, b = opt$par$b, phi = opt$par$phi ) 
}

fit <- rstan::sampling(model, data = stan_data, chains = 4, iter = 30000, warmup = 9000, thin = 100, init = initf)
